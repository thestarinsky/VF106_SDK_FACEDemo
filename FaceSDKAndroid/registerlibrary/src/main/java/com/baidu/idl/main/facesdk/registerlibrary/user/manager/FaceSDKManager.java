...
//    }
    ...
    public ImportFeatureResult getFeature(Bitmap bitmap, byte[] feature, BDFaceSDKCommon.FeatureType featureType) {
        ...
        // 最大检测人脸，获取人脸信息
        FaceInfo[] faceInfos = getFaceDetect().detect(BDFaceSDKCommon.DetectType.DETECT_VIS, imageInstance);
        if (faceInfos == null || faceInfos.length == 0) {
            ...
            // 最大检测人脸，获取人脸信息
            faceInfos = getFaceDetect().detect(BDFaceSDKCommon.DetectType.DETECT_VIS, imageInstance);
            // 若外扩后还未检测到人脸，则旋转图片检测
            if (faceInfos == null || faceInfos.length == 0) {
        ...
        // 判断质量
        /*boolean quality = onQualityCheck(faceInfo);
        if (!quality) {
            imageInstance.destory();
            return new ImportFeatureResult(0, null);
        }*/
        // 人脸识别，提取人脸特征值
        float ret = getFaceFeature().feature(featureType, imageInstance, faceInfo.landmarks, feature);
        // 人脸抠图
//        BDFaceImageInstance cropInstance = getFaceCrop().cropFaceByLandmark(imageInstance, faceInfo.landmarks, 2.0f, true, new AtomicInteger());
//        if (cropInstance == null) {
//            imageInstance.destory();
//            return new ImportFeatureResult(10, null);
//        }
//
//        Bitmap cropBmp = BitmapUtils.getInstanceBmp(cropInstance);
