...
public class FaceSDKManager {
    ...
    private FaceDetect faceTrackDetect;

    public FaceDetect getFaceTrackDetect() {
        return faceTrackDetect;
    }
    ...
    public void initModel(final Context context, final SdkInitListener listener) {
        ...
        BDFaceInstance bdFaceTrackInstance = new BDFaceInstance();
        bdFaceTrackInstance.creatInstance();
        faceTrackDetect = new FaceDetect(bdFaceTrackInstance);
        // 红外检测
        ...
        faceMouthMask.initModel(context,
                "face-sdk-models/mouth_mask/mouth_mask-customized-pa-rk3568.model.float32-2.0.1.3",
                new Callback() {
                    @Override
        ...
        faceDetectPerson.initQuality(context,
                ...
                GlobalSet.OCCLUSION_MODEL, new Callback() {
                    ...
                    public void onResponse(int code, String response) {
                        if (code != 0 && listener != null) {
//                            L.e("初始化模型失败 faceDetectPerson  initQuality ");
                            listener.initModelFail(code, response);
                        }
                    }
                });
        faceTrackDetect.initModel(context,
                GlobalSet.DETECT_VIS_MODEL,
                GlobalSet.DETECT_PRIOR_BOX,
...
//                            ToastUtils.toast(context, "模型加载完毕，欢迎使用");
        ...
        future3 = es3.submit(new Runnable() {
            ...
            public void run() {
                synchronized (FaceApi.getInstance().getAllUserList()) {
                    FaceSDKManager.getInstance().getFaceSearch().pushPoint(
    ...
    public boolean initConfig() {
        if (faceDetect != null && faceTrackDetect != null && faceDetectPerson != null) {
...
//            // TODO: 模糊，遮挡，光照三个质量检测和姿态角查默认关闭，如果要开启，设置页启动
            ...
            faceTrackDetect.loadConfig(config);
    ...
    public void onDetectCheck(final byte[] rgbData,
        ...
        // 【【提取特征+1：N检索：3】
        onDetectCheck(rgbData, nirData, depthData, srcHeight, srcWidth, liveCheckMode, 3, 0, faceDetectCallBack);
    }
    ...
    public void onDetectCheck(final byte[] rgbData,
        ...
        // 【【提取特征+1：N检索：3】
        onDetectCheck(rgbData, nirData, depthData, srcHeight, srcWidth, liveCheckMode, 3, i, faceDetectCallBack);
    }
...
//                File[] picFiles1 = picFiles[i].listFiles();
                Bitmap bitmap = BitmapUtils.broadImage(BitmapFactory.decodeFile(picFiles[i].getAbsolutePath()));
//            method1(FileUtils.getSDRootFile() + "/" + "testbyte.txt" , getBitmapByte(bitmap));
...
//                }
                ...
                FaceInfo[] faceInfos = FaceSDKManager.getInstance().getFaceTrackDetect()
                ...
                if (faceInfos != null && faceInfos.length > 0) {
                    Log.e("Aaaaaaaa", faceInfos[0].score + "");
            ...
            public void run() {
                ...
                FaceInfo[] faceInfos = FaceSDKManager.getInstance()
                        ...
                        .detect(BDFaceSDKCommon.DetectType.DETECT_VIS,
                                BDFaceSDKCommon.AlignType.BDFACE_ALIGN_TYPE_RGB_ACCURATE,
                                rgbInstance,
                                /*fastFaceInfos*/null, bdFaceDetectListConfig);
                ...
                if (faceInfos != null && faceInfos.length > 0) {
                    ...
                    livenessModel.setLandmarks(faceInfos[0].landmarks);
                    Log.e("test_face", Arrays.toString(faceInfos[0].landmarks));
                ...
                float[] scores = faceMouthMask.checkMask(rgbInstance, faceInfos);
...
//        }
    ...
    private void featureSearch(final int featureCheckMode,
        ...
        if (featureSize == FEATURE_SIZE / 4) {
            ...
            if (featureResult != null && featureResult.size() > 0) {
                ...
                Log.e("facetest", topFeature.getScore() + "");
                ...
                if (topFeature != null && topFeature.getScore() >
                        threholdScore) {
                    Log.e("faceLock", "id: " + topFeature.getId() + ",score: " + topFeature.getScore());
    ...
    // 人证核验
    public float personDetect(final Bitmap bitmap, final byte[] feature) {
        BDFaceImageInstance rgbInstance = new BDFaceImageInstance(bitmap);
        float ret = -1;
...
//    }
     ...
     * @param feature 特征点
     */
    public boolean featureSearch(byte[] feature) {
        // 特征提取成功
        // TODO 阈值可以根据不同模型调整
        ArrayList<Feature> featureResult = getFaceFeature().featureSearch(feature, BDFaceSDKCommon.FeatureType.BDFACE_FEATURE_TYPE_LIVE_PHOTO, 1, true);
        ...
        if (featureResult != null && featureResult.size() > 0) {
            ...
            if (topFeature != null && topFeature.getScore() > threholdScore) {
                // 当前featureEntity 只有id+feature 索引，在数据库中查到完整信息
//                Feature featureBean = FaceApi.getInstance().getPersonFaceById(topFeature.getId());
//                return featureBean != null;
                return false;
            } else {
                return false;
            }
